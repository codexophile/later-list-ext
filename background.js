...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message?.type === 'laterlist:getData') {
        getData().then(data => sendResponse({ data })).catch(() => sendResponse({ data: DEFAULT_DATA }));
        return true;
    }
    if (message?.type === 'laterlist:addLink') {
        addLink(message.payload).then(link => sendResponse({ link })).catch(err => sendResponse({ error: err?.message }));
        return true;
    }
    if (message?.type === 'laterlist:setData') {
        saveData(message.payload).then(() => sendResponse({ ok: true })).catch(err => sendResponse({ error: err?.message }));
        return true;
    }
    return false;
});